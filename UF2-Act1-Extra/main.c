...
int	main(void)
{
	...
	srand(time(NULL));
	printf("==================================================================================\n");
	printf("Ingrese la cantidad de cartas con las que desean jugar: ");
	scanf("%d", numero_cartas);
	jugador1 = (Carta *)malloc(numero_cartas * sizeof(Carta));
	if (!jugador1)
		return (NULL);
	jugador2 = (Carta *)malloc(numero_cartas * sizeof(Carta));
	if (!jugador2)
		return (NULL);
	turno = rand() % 2;
	IniciarCartas(jugador1, numero_cartas);
	IniciarCartas(jugador2, numero_cartas);
	while (opc != 0)
	{
		system("cls");
		printf("Jugador # 1: \n");
		MostrarCartas(jugador1, numero_cartas);
		printf("Vida Promedio : %.2f \n\n", CalculaVidaPromedio(jugador1));
		printf("Jugador # 2: \n");
		MostrarCartas(jugador2, numero_cartas);
